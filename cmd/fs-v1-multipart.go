...
func (fs *FSObjects) ListObjectParts(ctx context.Context, bucket, object, uploadID string, partNumberMarker, maxParts int) (result ListPartsInfo, e error) {
	...
	actualSizesMap := make(map[int]int64)
	for _, entry := range entries {
		if entry == fs.metaJSONFile {
			continue
		}
		partNumber, etag1, actualSize, derr := fs.decodePartFile(entry)
		actualSizesMap[partNumber] = actualSize
		if derr != nil {
		...
		stat1, serr := fsStatFile(ctx, pathJoin(uploadIDDir, fs.encodePartFile(partNumber, etag1, actualSize)))
		if serr != nil {
			return result, toObjectErr(serr)
		}
		stat2, serr := fsStatFile(ctx, pathJoin(uploadIDDir, fs.encodePartFile(partNumber, etag2, actualSize)))
		if serr != nil {
			return result, toObjectErr(serr)
		}
	...
	for partNumber, etag := range partsMap {
		parts = append(parts, PartInfo{PartNumber: partNumber, ETag: etag, ActualSize: actualSizesMap[partNumber]})
	}
...
func (fs *FSObjects) CompleteMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, parts []CompletePart) (oi ObjectInfo, e error) {
	...
	// Returns the file name which matches the partNumber and etag.
	getPartFile := func(entries []string, partNumber int, etag string) string {
		for _, entry := range entries {
