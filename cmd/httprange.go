...
func (h *HTTPRangeSpec) ContentRangeString(resourceSize int64) string {
	// FIXME: Error is ignored below - it is assumed that resourceSize is
	// valid for the given range specification.
	start, rangeLength, _ := h.GetOffsetLength(resourceSize)
...
func parseRequestRangeSpec(rangeString string) (hrange *HTTPRangeSpec, err error) {
	...
	if len(offsetBeginString) > 0 {
		if offsetBeginString[0] == '+' {
			return nil, fmt.Errorf("Byte position ('%s') must not have an sign", offsetBeginString)
	...
	if len(offsetEndString) > 0 {
		if offsetEndString[0] == '+' {
			return nil, fmt.Errorf("Byte position ('%s') must not have an sign", offsetEndString)
