...
func (o *ObjectInfo) GetDecryptedRange(rs *HTTPRangeSpec) (encOff, encLength, skipLen int64, seqNumber uint32, partStart int, err error) {
	...
	if crypto.IsMultiPart(o.UserDefined) {
		...
		for _, part := range o.Parts {
			...
			sizes = append(sizes, int64(partSize))
	...
	for i = 0; i < len(sizes); i++ {
		if off < cumulativeSum+sizes[i] {
			partStart = i
			break
		}
		cumulativeSum += sizes[i]
		encPartSize, _ := sio.EncryptedSize(uint64(sizes[i]))
	...
	for i = partStart; i < len(sizes); i++ {
		if endOffset < cumulativeSum+sizes[i] {
			partEnd = i
			break
		}
		cumulativeSum += sizes[i]
