...
func (xl xlObjects) GetObjectNInfo(ctx context.Context, bucket, object string, rs *HTTPRangeSpec) (objInfo ObjectInfo, reader io.ReadCloser, err error) {
	...
	if rs != nil {
		if objInfo.IsCompressed() {
			startOffset, readLength = rs.GetOffsetLength(objInfo.GetActualSize())
			// Reducing the offset and length
			// Incase of range based queries, the offset is reduced.
			startOffset, _ = getCompressedOffsets(objInfo, startOffset)
			readLength = objInfo.Size - startOffset
		} else {
	...
	for partIdx := 1; ; partIdx++ {
		...
		if n == 0 && data.Size() == -1 {
			...
			for _, disk := range xl.getDisks() {
				dErr := disk.DeleteFile(minioMetaTmpBucket, tempErasureObj)
				if dErr != nil {
