...
func (api objectAPIHandlers) ListObjectsV2Handler(w http.ResponseWriter, r *http.Request) {
	...
	for i := range listObjectsV2Info.Objects {
		...
		if isCompressed(listObjectsV2Info.Objects[i].UserDefined) {
			// Read the decompressed size from the meta.json.
			actualSize = getDecompressedSize(listObjectsV2Info.Objects[i])
			if actualSize < 0 {
...
func (api objectAPIHandlers) ListObjectsV1Handler(w http.ResponseWriter, r *http.Request) {
	...
	for i := range listObjectsInfo.Objects {
		var actualSize int64
		if isCompressed(listObjectsInfo.Objects[i].UserDefined) {
			// Read the decompressed size from the meta.json.
			actualSize = getDecompressedSize(listObjectsInfo.Objects[i])
			if actualSize < 0 {
				writeErrorResponse(w, ErrInvalidDecompressedSize, r.URL)
				return
			}
