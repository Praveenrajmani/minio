...
Minio server has compression disabled by default. By enabling it, Minio ensures efficient disk space usage by compressing the objects before being written to disk(s). 
...
Compression can be enabled by updating the `compress` config settings for Minio server config. Compression settings take extensions and mime-types which needs to be compressed.
...
Compression can be enabled by simply setting the `enabled` option to `true`. Since text, log, csv, json files are highly compressible, These extensions/mime-types are included by default for compression.
